...
foreach ($this->viewData as $key => $value) {
    echo $value[$this->tablePrimaryKey];
	++$idx_;
	if($idx_<$count_){
		echo ",";
	}
...
?>
    function doCommand(action){
        ...
        document.forms.<?php $this->eFormName(); ?>.elements.id.value=id;
        ...
        function doOrderSave(key){
            var this_=document.forms.<?php $this->eFormName(); ?>;
                this_.elements["action"].value="order-save";
                this_.elements["order"].value=key;            
                this_.elements["id"].value="<?php
    foreach ($this->viewData as $key => $value) {
        echo $value[$this->tablePrimaryKey] . ";";
    }
    ?>";
...
function doOrderUp(field,key) {
	var this_=document.forms.<?php $this->eFormName(); ?>;
		this_.elements["action"].value="order-up";
		this_.elements["order"].value=key;
		this_.elements["id"].value=field;
		this_.submit();	
}

function doOrderDown(field,key) {
            var this_=document.forms.<?php $this->eFormName(); ?>;           
		this_.elements["action"].value="order-down";
		this_.elements["order"].value=key;
		this_.elements["id"].value=field;
		this_.submit();
}
...
function doToggle(field,key) {
            var this_=document.forms.<?php $this->eFormName(); ?>;           
		this_.elements["action"].value="toggle";
...
function doRadio(field,key) {
            var this_=document.forms.<?php $this->eFormName(); ?>;           
		this_.elements["action"].value="radio";
...
function doValueSave(key){
            var this_=document.forms.<?php $this->eFormName(); ?>;           
                this_.elements["action"].value="value-save";
        ...
        <tbody>
            ...
            foreach ($this->viewData as $key => $value) {
                ...
                foreach ($this->tableHead as $key_ => $value_) {
                    ...
                    if ($key_ === "#") {
				if($value["@write"]){
	                        	echo "<input type=\"checkbox\" id=\"cbox_" . $value["#"] . "\"  name=\"id_" . $value[$this->tablePrimaryKey] . "\" value=\"" . $value[$this->tablePrimaryKey] . "\" />";
				}else{
					echo "&#160;";
				};
                    } else
			...
			if($this->tableType[$key_][0]=="toggle"){
				...
				if($value["@write"]){
					...
					echo "<a href=\"#\" style=\"text-decoration:none;\" onclick=\"javascript:doToggle('" . $value[$this->tablePrimaryKey] . ";','" . $key_ . "');return false;\">";
			...
			if($this->tableType[$key_][0]=="radio"){
				...
				if($value["@write"]){
					...
					echo "<a href=\"#\" style=\"text-decoration:none;\" onclick=\"javascript:doRadio('" . $value[$this->tablePrimaryKey] . ";','" . $key_ . "');return false;\">";
					echo $toggle_img_[$img];
					echo "</a>";
			...
			if($this->tableType[$key_][0]=="order"){
				...
				echo "<div class=\"input-group\" style=\"width:12em;\">";
		        		echo "<input type=\"text\"";
			                       echo " name=\"order_" . $value[$this->tablePrimaryKey] . "\"";
			                       echo " value=\"" . $value[$key_] . "\"";
			                       echo " size=\"4\"";
		        		       echo "class=\"form-control\"></input>";
					echo "<span class=\"input-group-btn\">";
						echo "<button type=\"button\" class=\"btn btn-default\" onclick=\"javascript:doOrderUp('" . $value[$this->tablePrimaryKey] . ";','" . $key_ . "');return false;\"><i class=\"fa fa-chevron-up\"></i></button>";
						echo "<button type=\"button\" class=\"btn btn-default\" onclick=\"javascript:doOrderDown('" . $value[$this->tablePrimaryKey] . ";','" . $key_ . "');return false;\"><i class=\"fa fa-chevron-down\"></i></button>";
					echo "</span>";
				echo "</div>";
			...
			if($this->tableType[$key_][0]=="value"){
	                        echo "<input type=\"text\"";
        	                echo " name=\"value_" . $value[$this->tablePrimaryKey] . "\"";
                	        echo " value=\"" . $value[$key_] . "\"";
                        	echo " size=\"8\"";
