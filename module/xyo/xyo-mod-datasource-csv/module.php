...
//
$className = "xyo_mod_datasource_Csv";

class xyo_mod_datasource_Csv extends xyo_Module {
	...
	function __construct(&$object, &$cloud) {
		...
		if ($this->isBase("xyo_mod_datasource_Csv")) {
	...
	function setConnection($name, $databasePath) {
		$this->connectionList_[$name] = new xyo_mod_datasource_csv_Connection($this, $name, $databasePath);
	}
						...
						if ($this->dataSourceList_[$name]) {
							if (strcmp($matches[2], "table") == 0) {
								if ($this->connectionList_[$matches[1]]->open()) {
									$v_ = new xyo_mod_datasource_csv_Table($this, $this->connectionList_[$matches[1]], $matches[3], $name, $this->dataSourceList_[$name], $as_);
									if ($v_->isOk()) {
							...
							if (strcmp($matches[2], "query") == 0) {
								if ($this->connectionList_[$matches[1]]->open()) {
									$v_ = new xyo_mod_datasource_csv_Query($this, $this->connectionList_[$matches[1]], $matches[3], $name, $this->dataSourceList_[$name], $as_);
									if ($v_->isOk()) {
