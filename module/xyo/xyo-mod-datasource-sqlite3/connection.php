...
class xyo_mod_datasource_sqlite3_Connection {
    ...
    function sqlite3Query_($query) {
        if ($this->log) {

            $retV = $this->db->query($query);

            $fs = fopen($this->cloud->get("path_log") . "sqlite3." . $this->name . ".log", "ab");
            if ($fs) {
    ...
    function sqlite3QuerySingle_($query) {
        if ($this->log) {
    ...
    function queryDirect($query) {
        $result = $this->sqlite3Query_($query);
        if (!$result) {
            $this->debug2($query);
            $result = null;
        };
    ...
    function queryDirectSingle($query) {
        $result = $this->sqlite3QuerySingle_($query);
        if (!$result) {
            $this->debug2($query);
            $result = null;
        };
