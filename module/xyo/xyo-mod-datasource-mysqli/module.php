...
$className = "xyo_mod_datasource_Mysqli";

class xyo_mod_datasource_Mysqli extends xyo_Module {
	...
	function __construct(&$object, &$cloud) {
		...
		if ($this->isBase("xyo_mod_datasource_Mysqli")) {
	...
	function setConnection($name, $user, $password, $server, $port, $database, $prefix) {
		$this->connectionList_[$name] = new xyo_mod_datasource_mysqli_Connection($this->cloud, $this, $name, $user, $password, $server, $port, $database, $prefix);
	}
							...
							if (strcmp($matches[2], "table") == 0) {
								if ($this->connectionList_[$matches[1]]->open()) {
									...
									if ($this->connectionList_[$matches[1]]->useDb()) {
										$v_ = new xyo_mod_datasource_mysqli_Table($this, $this->connectionList_[$matches[1]], $matches[3], $name, $this->dataSourceList_[$name], $as_);
										if ($v_->isOk()) {
								...
								if ($this->connectionList_[$matches[1]]->open()) {
									if ($this->connectionList_[$matches[1]]->useDb()) {
										$v_ = new xyo_mod_datasource_mysqli_Query($this, $this->connectionList_[$matches[1]], $matches[3], $name, $this->dataSourceList_[$name], $as_);
										if ($v_->isOk()) {
