...
class xyo_mod_datasource_sqlite_Query extends xyo_Config {
    ...
    function &copyThis() {
        $retV = new xyo_mod_datasource_sqlite_Query($this->module_, $this->connection_, $this->name_, $this->datasource_, $this->descriptor_, $this->as_, false);
        if ($retV) {
            ...
            foreach ($this->fieldOrder_ as $key_ => $value_) {
                if ($value_) {
                    if ($order) {
                        ...
                        if ($value_ == $this->orderDescendent) {
                            $order.=" DESC";
                        };
                    } else {
    ...
    function querySetOrderLoad_() {
        ...
        foreach ($x_ as $key_ => $value_) {
            ...
            if (array_key_exists($value_, $this->querySecondTable_)) {
    ...
    function load($start=null, $length=null) {
        ...
        if ($this->result_) {
            $fieldValue_ = sqlite_fetch_array($this->result_, SQLITE_ASSOC);
            if ($fieldValue_) {
				if(!empty($this->fieldSelect_)){$this->setEmptyValue();};
	...
	function hasNext(){
		if ($this->result_) {
			$this->hasNextValue_ = sqlite_fetch_array($this->result_, SQLITE_ASSOC);
