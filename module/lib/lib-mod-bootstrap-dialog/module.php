...
class lib_mod_BootstrapDialog extends xyo_Module {
    ...
    public function __construct(&$object, &$cloud) {
        ...
        if ($this->isBase("lib_mod_BootstrapDialog")) {
            ...
            if ($htmlHead) {
                $htmlHead->setCss($this->name,$this->pathBase."media/sys/css/bootstrap-dialog.min.css");
            } else {
                $this->moduleDisable();
            };
            $htmlFooter = &$cloud->getModule("xyo-mod-htmlfooter");
            if ($htmlFooter) {
                $htmlFooter->setJs($this->name,$this->pathBase."media/sys/js/bootstrap-dialog.min.js");
            } else {
                $this->moduleDisable();
            };
        }
