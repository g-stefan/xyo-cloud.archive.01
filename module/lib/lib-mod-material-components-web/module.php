...
class lib_mod_MaterialComponentsWeb extends xyo_Module {
    ...
    public function __construct(&$object, &$cloud) {
        ...
        if ($this->isBase("lib_mod_MaterialComponentsWeb")) {
            ...
            if ($htmlHead) {
                $htmlHead->setCss($this->name,$this->site."media/sys/css/material-components-web.min.css");
            } else {
                $this->moduleDisable();
            };
            $htmlFooter = &$cloud->getModule("xyo-mod-htmlfooter");
            if ($htmlFooter) {
                $htmlFooter->setJs($this->name,$this->site."media/sys/js/material-components-web.min.js");
            } else {
                $this->moduleDisable();
            };
        }
