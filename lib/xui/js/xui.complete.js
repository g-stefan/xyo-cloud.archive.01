...
(function(){
	...
	this.init=function(){
		...
		this.processEvent = function(){
			this_.currentState = ("" + this_.elResponsiveAfter.getPropertyValue("content")).replace(/["']/g, "");
			if(this_.lastState == this_.currentState){
				return;
			};	
			var customEvent;		
			try{			
				customEvent = new CustomEvent("xui-responsive", { detail: {state: this_.currentState, last: this_.lastState } });
			}catch(e){
				// IE11
				customEvent=document.createEvent("CustomEvent");
				customEvent.initCustomEvent("xui-responsive", false, false, {state: this_.currentState, last: this_.lastState });
			};
...
(function(){
	...
	this.processResponsive=function(state,last){

		var elList=XUI.getByClassName(document,"xui dashboard size-x64");	
		for(var elIndex=0; elIndex<elList.length; ++elIndex){
	...
	this.cookieUpdateState=function(newState){
		var cookieState=XUI.getCookie("xui-dashboard-size-x64");
		if(cookieState){
			var mode;
			var state;
			...
			mode=scan[0];
			state=newState;
			XUI.setCookie("xui-dashboard-size-x64",mode+"-"+state);
		};		
	}
	...
	this.toogleAction=function(){
		var el=XUI.getByClassNameFirst(document,"xui dashboard size-x64");
		if(el){
	...
	this.init=function(){
		this.cookieOk=false;
		window.addEventListener("xui-responsive", this.notifyResponsive);
		this.processResponsive(XUI.Responsive.currentState,XUI.Responsive.lastState);
		var cookieState=XUI.getCookie("xui-dashboard-size-x64");
		if(cookieState){
			var mode;
			var state;
...
(function(){
	...
	this.cookieUpdateState=function(newState){
		...
		if(cookieState){
			...
			mode=scan[0];
			state=newState;
			XUI.setCookie("xui-dashboard-size-x48",mode+"-"+state);
		};		
	}
