...
if($this->isAjax()){
	...
	$this->setHtmlJsSource("\$(\"#".$this->getElementId($element)."\").fileinput({showUpload: false});","load");
};
...
<div class="form-group<?php if($this->isElementError($element)){echo " has-error";}; ?>">
	...
	<td class="xyo-form-file-link-delete">
		<span class="xyo-form-file-link-delete-sub" onclick="$('#<?php $this->eElementId($element); ?>_delete').val(1);$('#<?php $this->eElementId($element); ?>_info').html('<span><?php echo $this->getFromLanguage("file_link_no_file"); ?></span>');return false;"><i class="glyphicon glyphicon-remove"></i></span>
	</td>
	</tbody>
	</table>
